...
folder <- "D:/samples/breast_cancer/data"
folder.healthy = "D:/samples/breast_cancer/cx_report/healthy"
setwd(folder)

num.cores <- 12


sample.id <- list.files(path = folder.healthy, pattern = "SRR8117", full.names = FALSE)
sample.id
files <- paste0(folder.healthy, "/", sample.id , "/", sample.id, ".cytosine_report.txt.gz")
...
Normal <- readCounts2GRangesList(filenames = files,
                             sample.id = sample.id,
                             columns = c(seqnames = 1, start = 2, strand = 4,
                                         mC = 5, uC = 6, context=7, signal=8),
                             remove = FALSE, pattern = "^1[^0123456789_]",
                             nThread = num.cores,
                             verbose = TRUE)
save(Normal, file = paste0(folder, "/chr1/meth_healthy_chr1.RData"),compress = "xz")
